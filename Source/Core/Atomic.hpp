...
namespace Mach::Core {
	...
	public:
		...
		MACH_ALWAYS_INLINE Atomic(const Atomic& rhs) noexcept
			: m_atomic(rhs.m_atomic.load(std::memory_order_relaxed)) {}
		MACH_ALWAYS_INLINE Atomic& operator=(const Atomic& rhs) noexcept {
			const auto order = std::memory_order_relaxed;
			m_atomic.store(rhs.m_atomic.load(order), order);
			return *this;
		}
		MACH_ALWAYS_INLINE Atomic(Atomic&& rhs) noexcept : m_atomic(rhs.m_atomic.load(std::memory_order_relaxed)) {}
		MACH_ALWAYS_INLINE Atomic& operator=(Atomic&& rhs) noexcept {
			const auto order = std::memory_order_relaxed;
			m_atomic.store(rhs.m_atomic.load(order), order);
			return *this;
		}

		MACH_ALWAYS_INLINE void store(T desired, Order order = Order::SeqCst) const noexcept {
			m_atomic.store(desired, to_std(order));
		}
		MACH_NO_DISCARD MACH_ALWAYS_INLINE T load(Order order = Order::SeqCst) const noexcept {
			return m_atomic.load(to_std(order));
		}
		MACH_NO_DISCARD MACH_ALWAYS_INLINE T exchange(T desired, Order order = Order::SeqCst) const noexcept {
			return m_atomic.exchange(desired, to_std(order));
		}

		MACH_NO_DISCARD MACH_ALWAYS_INLINE Option<T>
		...
		MACH_ALWAYS_INLINE T fetch_add(T arg, Order order = Order::SeqCst) const noexcept {
			return m_atomic.fetch_add(arg, to_std(order));
		}
		MACH_ALWAYS_INLINE T fetch_sub(T arg, Order order = Order::SeqCst) const noexcept {
			return m_atomic.fetch_sub(arg, to_std(order));
		}
		MACH_ALWAYS_INLINE T fetch_and(T arg, Order order = Order::SeqCst) const noexcept {
			return m_atomic.fetch_and(arg, to_std(order));
		}
		MACH_ALWAYS_INLINE T fetch_or(T arg, Order order = Order::SeqCst) const noexcept {
			return m_atomic.fetch_or(arg, to_std(order));
		}
		MACH_ALWAYS_INLINE T fetch_xor(T arg, Order order = Order::SeqCst) const noexcept {
			return m_atomic.fetch_xor(arg, to_std(order));
		}

	private:
		MACH_ALWAYS_INLINE std::memory_order to_std(Order order) const {
